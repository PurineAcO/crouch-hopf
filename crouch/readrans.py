...
def read_cells(ransdata:str,S_MAX:int,N_MAX:int,h:int = cc.HALO):
    ...
    for k in range(len(data)):
        ...
        cc.CellList[i + h][j + h] = cc.cell_class((i, j), *data[k, [col["x"], col["y"], col["rho"], col["u"],
                                                            col["v"],col["T"],col["miubl"],col["vol"],col["sad"]]])
...
def fill_ghost(S_MAX:int,N_MAX:int,h:int = cc.HALO):
    ...
    for k in range(1, h + 1):
        for s in range(1, S_MAX + 1):
            ...
            cc.CellList[s + h][1 - k + h] = cc.cell_class((s, k), 0.0, 0.0,
                                                          c.rho, -c.u, -c.v, c.T, -c.miubl,c.vol,c.sad)
    # 远场虚层: 对称
    for k in range(1, h + 1):
        for s in range(1, S_MAX + 1):
    ...
    for n in range(1, N_MAX + 1):
        for k in range(1, h + 1):
            ...
            cc.CellList[h + 1 - k][n + h] = cc.cell_class((S_MAX + 1 - k, n), 0.0, 0.0,c_hi.rho,c_hi.u, 
                                                          c_hi.v, c_hi.T, c_hi.miubl,c_hi.vol,c_hi.sad)
            c_lo = cc.CellList[k + h][n + h]
            cc.CellList[S_MAX + k + h][n + h] = cc.cell_class((k, n), 0.0, 0.0,c_lo.rho, c_lo.u, c_lo.v,
