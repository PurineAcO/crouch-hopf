...
class cell_class:
  ...
  def sa_convect_vec(self):
    ...
    return (
      np.array([self.u * self.miubl, self.rho * self.miubl, 0, 0, self.rho * self.u]),
      np.array([self.v * self.miubl, 0, self.rho * self.miubl, 0, self.rho * self.v]),
    )


class face_class:
  def __init__(self, direction: str, mid: tuple, jacobi, me: cell_class, nei: cell_class):
