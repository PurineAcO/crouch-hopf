...
def read_cells(ransdata: str, S_MAX: int, N_MAX: int, h: int = cc.HALO):
  ...
  for k in range(len(data)):
    ...
    cc.CellList[i + h][j + h] = cc.cell_class(
      (i, j),
      *data[
        k,
        [
...
def fill_ghost(S_MAX: int, N_MAX: int, h: int = cc.HALO):
  ...
  for k in range(1, h + 1):
    for s in range(1, S_MAX + 1):
      ...
      cc.CellList[s + h][1 - k + h] = cc.cell_class(
        (s, 1 - k), 0.0, 0.0, c.rho, -c.u, -c.v, c.T, -c.miubl, c.vol, c.sad
      )
  # 远场虚层: 对称
  for k in range(1, h + 1):
    for s in range(1, S_MAX + 1):
      c = cc.CellList[s + h][N_MAX + 1 - k + h]
      cc.CellList[s + h][N_MAX + k + h] = cc.cell_class(
        (s, N_MAX + k), 0.0, 0.0, c.rho, c.u, c.v, c.T, c.miubl, c.vol, c.sad
      )
  # 周期虚列: 循环
  for n in range(1, N_MAX + 1):
    for k in range(1, h + 1):
      c_hi = cc.CellList[S_MAX + 1 - k + h][n + h]
      cc.CellList[h + 1 - k][n + h] = cc.cell_class(
        (S_MAX + 1 - k, n),
        0.0,
        0.0,
        c_hi.rho,
      ...
      cc.CellList[S_MAX + k + h][n + h] = cc.cell_class(
        (k, n), 0.0, 0.0, c_lo.rho, c_lo.u, c_lo.v, c_lo.T, c_lo.miubl, c_lo.vol, c_lo.sad
      )
  ...
  with open(edgedata, encoding='utf-8') as f:
    ...
    for line in f:
      ...
      if etype == 'NS':  # 处理NS面的时候,需要考虑远场壁面的虚单元,但是处理WE面时直接回绕了
        nei = (
          cc.CellList[s + h][h] if c1_n == 0 else cc.CellList[c1_s + h][c1_n + h]
        )  # 源文件要求0是边界符号
