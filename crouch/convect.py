...
def cell_at(i:int,j:int):
    """按结构化索引 (i,j)=(s,n) 取单元, 含 halo 偏移 (CellList[s+HALO][n+HALO]) """
    return cc.CellList[i + cc.HALO][j + cc.HALO]
...
def face_convect_mat_4th_mid(face:cc.face_class):
    ...
    if face.direction == 'N':
        G_list = [face.jacobi(cell_at(i, dj).F, cell_at(i, dj).G)[1] for dj in [j+2, j+1, j, j-1]]
        ...
        F_list = [face.jacobi(cell_at(di, j).F, cell_at(di, j).G)[0] for di in [i+2, i+1, i, i-1]]
...
def face_convect_mat_3rd_upwind(face:cc.face_class):
    ...
    if face.direction == 'N':
        if v_n <= 0:
            G_list = [face.jacobi(cell_at(i, dj).F, cell_at(i, dj).G)[1] for dj in [j+2, j+1, j]]
            for pos,g,w in zip([1,3,7],G_list,[-1/6,5/6,1/3]):
                upwind_3rd[pos-1] = w*g
        else:
            G_list = [face.jacobi(cell_at(i, dj).F, cell_at(i, dj).G)[1] for dj in [j-1,j,j+1]]
        ...
        if v_n <= 0:
            F_list= [face.jacobi(cell_at(di, j).F, cell_at(di, j).G)[0] for di in [i+2,i+1,i]]
            for pos,g,w in zip([9,8,7],F_list,[-1/6,5/6,1/3]):
                upwind_3rd[pos-1] = w*g
        else:
            F_list= [face.jacobi(cell_at(di, j).F, cell_at(di, j).G)[0] for di in [i-1,i,i+1]]
